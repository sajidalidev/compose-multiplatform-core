...
private fun KeyboardEvent.toKey(): Key {
    // A TV names each remote button after the nearest PC keyboard key: on Tizen, Back arrives with
    // `code` "Escape" and the coloured buttons with "F1".."F4", while `keyCode` carries the real
    // vendor code (10009, 403..406). The remote's table therefore has to be consulted before the
    // string maps, or those aliases resolve to Escape/F1..F4 and shadow the TV buttons — observed
    // on a Tizen 9.0 set; the emulator delivers the same codes with an empty `code`. The table is
    // keyed on the TV actually hosting the app, so each TV sees only its own mapping and a desktop
    // browser skips it entirely, still reporting Unknown for the codes it shares with a remote
    // (19 is the PC "Pause" key).
    //
    // A keyboard attached to the TV is unaffected on Tizen, whose table holds no ordinary keyboard
    // key code. webOS is the one exception: its channel keys are 33/34, which are also
    // PageUp/PageDown, so on a webOS TV those two keyboard keys report ChannelUp/ChannelDown. The
    // remote wins deliberately — it is the input a TV app is built for.
    tvRemoteKeyFromKeyCode(currentTvPlatform, keyCode)?.let { return it }
