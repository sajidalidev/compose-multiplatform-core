...
private fun KeyboardEvent.toKey(): Key {
    // A Samsung TV names a remote button after the nearest PC keyboard key: Back arrives with
    // `code` "Escape" and the coloured buttons with "F1".."F4", while `keyCode` carries the real
    // Samsung code (10009, 403..406). The remote's table therefore has to be consulted before the
    // string maps, or those aliases resolve to Escape/F1..F4 and shadow the TV buttons — observed
    // on a Tizen 9.0 set; the emulator delivers the same codes with an empty `code`. A keyboard
    // attached to the TV is unaffected: its key codes (27, 112..115) are not in the table. Off-TV
    // the table is skipped entirely, so a desktop browser keeps reporting Unknown for the codes it
    // shares with the remote (e.g. 19, the PC "Pause" key).
    if (isTizenTv) {
