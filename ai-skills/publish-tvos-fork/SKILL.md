...
| Closure audit reports many `WARN` entries | Version-mismatch findings (the twin exists locally, just at a different version than requested). Do NOT add them to `compose-tvos-redirect/manifest/compose-tvos-versions.json` `mappings`; that file has no mappings by policy. Fix the version via the ledger and republish the exact JetBrains version. |
