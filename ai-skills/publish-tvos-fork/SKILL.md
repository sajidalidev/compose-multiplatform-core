...
The manual Central scripts are ledger-driven and per-module. Their versions come from the release ledger
`compose-tvos-redirect/tools/release/release-ledger/<version>.json` (generated by
`tools/release/ledger.py <tag> --verify`, never edited by hand), passed as `--ledger <json>`
or `$TVOS_LEDGER`. The ledger records, per library key, the exact JetBrains version (never
an androidx number) and, per artifact, which tvOS targets upstream actually ships as usable
leaf klibs and whether the `dev.sajidali` twin already exists on Central.

The flow, in order:

1. `python3 compose-tvos-redirect/tools/release/gate_prebuild.py --ledger <json> --worktree <this repo>`:
   refuses to start a publish unless upstream versions exist on Central, the worktree sits on
   the tagged upstream commit, Kotlin matches, the JDK is 21, no daemon holds the build,
   configuration cache is off, and the ledger is committed and fresh.
2. `python3 scripts/publish_set.py --ledger <json>` prints the derived publish set: every
   ledger artifact with EMPTY `tvos.usable` (advertised-but-404 counts as unusable and needs a
   fork), minus artifacts whose `forkOnCentral` is already true, minus the script's
   `NEVER_PUBLISH` list (ui-test, ui-test-junit4, ui-tooling, ui-tooling-data,
   ui-tooling-preview; `--include-tests` / `--include-tooling-preview` re-enable). Each
   member is resolved to its Gradle project path; artifacts that need a fork but are built by
   the compose-multiplatform repo (`components-*`) are listed separately. The script also
   emits the complete `-Pjetbrains.publication.version.<KEY>` list for all nine ledger
   libraries (omitting one makes the build stamp `9999.0.0-SNAPSHOT` into dependency edges)
   and aborts if any key has no version. `--only-group <org.jetbrains group>` narrows the set,
   `--ignore-central` shows the full rebuild view, `--format json|shell` is what the scripts
   consume.
3. `scripts/publish-tvos-fork.sh --ledger <json> [--only-group ...] [--dry-run]` publishes
   exactly that module list to mavenLocal by invoking each module's own
   `publish{KotlinMultiplatform,TvosArm64,TvosSimulatorArm64}PublicationToMavenLocal` and
   `jbVerifyDependencyVersions` tasks (the `publishComposeJbToMavenLocal` aggregate only
   understands library groups). `--modules` must be a subset of the derived set unless
   `--force-modules` is given, which warns loudly. `--dry-run` prints the exact Gradle
   command and runs nothing; always look at it before a real run.
4. `scripts/stage-central-bundle.sh --ledger <json> [--clean] [--dry-run]` runs the same
   per-module tasks with signing and assembles the bundle from the explicit module list
   (umbrella plus `-tvosarm64` / `-tvossimulatorarm64` leaves at the ledger version), not
   from a `find -name <version>` over `~/.m2`. A previous bundle is never deleted unless
   `--clean` is passed; without it the script refuses to overwrite.
5. `python3 compose-tvos-redirect/tools/release/gate_bundle.py` on the staged bundle, then the
   consumer probe (a throwaway tvOS consumer resolving the staged coordinates through the
   redirect plugin) before any human upload. `scripts/audit-tvos-closure.py` is deprecated
   (see its header) and is not part of this flow.

Policy reminder: the manifest `compose-tvos-redirect/manifest/compose-tvos-versions.json`
has `"mappings": {}` and must stay that way. The fork mirrors JetBrains coordinates 1:1 at
the identical version; a version mismatch anywhere is a release defect to fix by publishing
the right version, never something to paper over with a mapping.
...
What it does: wraps the per-module `publish<Platform>PublicationToMavenLocal` tasks for the
ledger-derived module list (see "Ledger-driven flow" above) with the fork's fixed property
set —
...
- `-Pjetbrains.publication.libraries=<library keys of the selected modules>`.
- A `-Pjetbrains.publication.version.<LIB>=<version>` property for EVERY library in the
  ledger, read from the ledger by `scripts/publish_set.py` (no hand-typed pins anywhere).

The stable publish set is selected per module from the ledger and the projects available in this checkout. The Reposilite development script retains its separate group-based flow.
...
| `WARN` | same as above, but the twin exists locally at a **different** version. Under the no-mappings policy this is a release defect (the twin must exist at the identical version, see the ledger), not a manifest input |
...
at least one `FAIL` or `UNKNOWN`. `WARN` entries are version mismatches: the script does not
fail on them, but they must NOT be turned into manifest `mappings` (the manifest has none and
never will). Treat each WARN as a wrong version somewhere and fix it via the ledger.
`--group-prefix` and `--repo-root` are overridable if auditing a non-default coordinate root
or repo location. This script is deprecated in favour of the ledger gates (see the
"Ledger-driven flow" section above).
...
`<REPOSILITE_URL>/dev/sajidali/compose/ui/ui/<COMPOSE version>-dev.<date>.<N>/ui-<...>.pom`
...
them as `WARN` when another local version exists, and as `FAIL` otherwise. A clean CI
repository can therefore expose gaps hidden by a developer’s Maven cache. Publish matching
twin versions before a Central release; keep the shipped manifest’s `mappings` empty.

**Pins.** The seven `VERSION_<LIB>` stamps and the `LIBRARIES` list live in
`scripts/tvos-versions.sh`, sourced by the Reposilite development script. These development
pins are separate from the manual Central release ledger.
...
**Flags.** `--dry-run` prints the Gradle command lines and exits without network calls or
credentials. `--local-only` builds and audits locally without uploading. It cannot be combined
with `--skip-local-audit`.
...
| `compose:material3:adaptive:adaptive:compileCommonMainKotlinMetadata` fails: `Unresolved reference 'window'` | adaptive resolves `WindowSizeClass` from upstream `org.jetbrains.androidx.window:window-core` (Maven coordinate in its build-fork.gradle), which ships tvOS; the fork no longer builds or publishes window-core. Check that coordinate still resolves and still has tvOS variants at the pinned version. |
...
| Publish looks successful but a `dev.sajidali` artifact you expect is missing from `~/.m2` | Check `-Pjetbrains.publication.libraries` actually lists it (as selected from the release ledger), and re-run the closure audit; a `FAIL` finding will name the exact missing coordinate. |
| Closure audit reports many `WARN` entries | These are version mismatches: another local version exists, but the requested twin does not. Resolve matching versions through the release ledger; do not add shipped manifest mappings. |
